...
/**
 * Одна заплатка шва «ветка → схождение J» в ЛОКАЛЬНЫХ координатах J
 * (x — поперёк, z — вдоль; линия входа в J = z 0). Ветка — расходящаяся дуга,
 * её торцевая кромка наклонена к входу J: внешний угол висит ПЕРЕД линией
 * входа (z<0 — дыра), внутренний — заходит внутрь J. Заплата — четырёхугольник
 * между кромкой ветки (обрезанной стенами J) и линией входа.
 */
export interface SeamPatchSpec {
  /** Цвет пола ветки (заплата визуально принадлежит ветке). */
  color: string;
  /** Торец ветки: внешний (обрезан стеной J) и внутренний углы кромки. */
  edgeA: Vec2;
  edgeB: Vec2;
  /** Те же u на линии входа J (z=0). */
  lineA: Vec2;
  lineB: Vec2;
  /**
   * Внешний угол торца ветки ЗА пределами ширины J (|x|>width/2) — между ним
   * и стеной J ставятся ограждения (перила по кромке + продолжение стены J
   * назад). null — кромка в пределах ширины J.
   */
  outerCorner: Vec2 | null;
}

/**
 * Шов стыка веток развилки с их схождением J. Заполняется генератором только
 * у J (TrackBuilder.computeSeam, без rng — детерминизм сохранён);
 * SectionChunk.buildSeam материализует пол-заплатки, ограждения и заднюю
 * стену J (закрывает проёмы в линию входа, за которыми бездна между ветками).
 */
export interface SeamSpec {
  patches: SeamPatchSpec[];
  /** Отрезки задней стены по линии входа J (между проёмами веток), J-локальные. */
  backWalls: { from: Vec2; to: Vec2 }[];
}
...
export interface SectionSpec {
  ...
  prevEnd2: Vec2;
  /**
   * Данные шва «ветки → схождение» в ЛОКАЛЬНЫХ координатах секции-схождения J
   * (u — поперёк, d — вдоль; d<0 — до линии входа в J). Заполняется генератором
   * только у J (см. TrackBuilder.computeSeam); SectionChunk.buildSeam строит по
   * ним заплатки пола и ограждения. Ветки — расходящиеся дуги, их торцевые
   * кромки НЕ параллельны входу J: без заплаток на стыке клиновидные дыры
   * (внутренние углы кромок висят перед входом J) и «языки» пола за шириной J.
   */
  seam?: SeamSpec | null;
